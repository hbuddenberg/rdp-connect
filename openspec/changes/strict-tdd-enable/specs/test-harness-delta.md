...
> **Capability**: `test-harness` (NEW) · **Change**: `strict-tdd-enable`
...
The helper MUST: source `lib/rdp-common.bash` from the repo root; call
`bats_require_minimum_version 1.5.0` at the top (R1 mitigation for bats skew);
provide `setup_test_home()` that creates `$BATS_TMPDIR/home`, exports `HOME`
to it, and returns the path (no test MUST touch the real `HOME`); provide
