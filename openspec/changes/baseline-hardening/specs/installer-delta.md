...
the dependency list (F1 removed them).
...
After deployment, the installer MUST run a smoke test that (a) invokes
`rdp-connect --help` (or equivalent no-op entrypoint) and confirms it exits `0`,
and (b) runs a parser unit probe that feeds a known-bad profile (containing a
non-allowlisted key) to `parse_env_safe` and confirms it is rejected. If either
check fails, the installer MUST exit non-zero with a message identifying which
smoke step failed.
...
The installer MUST emit a checksum manifest (e.g. `sha256sum` listing) of every
file it deployed, written to a documented location (e.g.
`~/.local/state/rdp/install-manifest.sha256`). The manifest MUST be reproducible:
running the installer twice on the same host produces identical manifest contents
(apart from any user-edited profile that the installer preserved).

#### Scenario: Manifest is generated and stable

- GIVEN a clean throwaway `HOME`
- WHEN the installer runs
- THEN a manifest file exists at the documented path listing every deployed file with its SHA-256
