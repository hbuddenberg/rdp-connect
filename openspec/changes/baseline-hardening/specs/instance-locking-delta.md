...
> **HIGH-RISK (F5 — credential-adjacent PID path):** the per-profile lock gates the
> RDP credential session. The current `/tmp/rdp-<profile>.pid` path is world-writable
> and shared across users — a symlink/DoS vector that can either block a legitimate
> session or trick the user into sending credentials to a spoofed window. The new
> path MUST be uid-private and live under `XDG_RUNTIME_DIR` (which is `0700` and
> per-user by definition on systemd distros). This requirement PAIRS with F9: the
> EXIT trap (engine-robustness) MUST clean the new path.
...
other's locks. The fallback to `/tmp` is permitted ONLY when `XDG_RUNTIME_DIR` is
unset; even under the fallback the uid suffix MUST be present.
...
### Requirement: EXIT trap cleans the new path (pairs with F9)
...
- THEN the trap completes without a "No such file" error (the `[ -f ]` guard from F9 covers this)
