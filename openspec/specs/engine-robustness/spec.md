...
> from `MON_FLAGS`/`DPI_FLAGS` to include `USB_FLAGS`, `DRIVE_FLAGS`, and
> `CLIPBOARD_FLAGS`. Corrected the suffix form from `"${ARR[@]-}"` to the
> no-suffix `"${ARR[@]}"` invariant documented in CLAUDE.md and enforced by
> `tests/freerdp3-flags.bats`.
...
> **Added by `usb-redirect-clipboard-windowrules` (2026-08-12).**
