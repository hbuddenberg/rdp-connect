...
> **Amendment (carry-forward Q5, resolved at archive):** the `test_helper.bash`
> skeleton above shows the version gate, path resolution, and the two helpers,
> but OMITS the `bats-support` / `bats-assert` external loader that task T1.3
> added during implementation. `assert_success`, `assert_output`, `assert_equal`,
> `assert_failure`, and `assert_empty` are NOT built into bats-core — they ship
> as separate repos in the bats-core org (and as separate `apt`/`dnf` packages).
> The deployed helper wraps the load in `_load_bats_assert()`, which searches
> (first match wins) `$BATS_LIB_PATH` → `/usr/lib/bats` → `~/.local/lib/bats`
> for `bats-support/load.bash` + `bats-assert/load.bash`, bails with a clear
> install hint (exit 2) if either is missing, then `load`s both. This loader
> runs AFTER `bats_require_minimum_version 1.5.0` and BEFORE path resolution.
> See `tests/test_helper.bash:42-77` for the live implementation and the README
> "Testing → Install bats-core + assertion libraries" distro matrix for the
> install patterns. (Documented here retroactively because the original design
> skeleton predated the loader decision.)
...
- [x] **Q1 — spec bug in `test-harness-delta.md`** (RESOLVED at archive): the spec said `verify-manifest` reads `~/.local/share/rdp/MANIFEST.sha256`, but the installer (`install-rdp-framework.sh:219`) + `bootstrap.sh:85` + Makefile all write/read `~/.local/state/rdp/manifest.sha256` (XDG `state/`, lowercase). Design follows the **installer** (source of truth). **Resolved by amending the spec at archive** — both occurrences (the `verify-manifest` requirement and the tamper scenario) now read `~/.local/state/rdp/manifest.sha256`, matching the deployed implementation.
