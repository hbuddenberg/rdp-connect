# hidpi-scaling Capability Spec

> The engine computes HiDPI desktop-scale flags for `xfreerdp3` without spawning
> `bc` or `python3`. Scale math uses `jq` (already a hard dependency) and bash
> arithmetic. This capability exists because the original implementation silently
> disabled HiDPI scaling on every minimal Arch/Omarchy install that did not pull
> `bc`, leaving users with blurry RDP at 200% scale and no warning.
